    ...
    <div id="content" class="clearfix fullwidth">
        ...
        <!-- <div class="list_status clearfix">
            <ul>
                ...
                <li>
                    <a @click="filterByStatus(CONSTANT.ORDER_STATUS.DA_KET_THUC)" style="cursor: pointer" class="black">
        ...
        <div class="gridtable">
            <table>
                <tbody>
                    <tr>
                        <td width="5%">STT</td>
                        <td width="15%">Mã đơn hàng/Ngày mua hàng</td>
                        <td width="40%">Thông tin sản phẩm</td>
                        <td width="20%">Thao tác</td>
                        <td width="15%">Gán NV mua</td>
                        <td width="5%">#</td>
                    </tr>
                    <tr v-for="(order, index) in orderList">
                        ...
                        <td>
                            <span class="uppercase">
                                <a class="blue" style="cursor: pointer; color: #0000ff" @click="viewDetail(order.id)">
                                    {{ order.orderCode }}
                                </a>
                            </span>
                            <p>
                        ...
                        <td>
                            <span class="blue">
                                {{ getStaffById(order.staffIdBuy) }}
                            </span>
                        </td>
                        <td>
                            <a style="cursor: pointer" @click="grantStaffToOrder(order)" class="button-link">Lưu</a>
                        </td>
                    </tr>
                </tbody>
            </table>
    ...
    methods: {
        async getListOrders() {
            ...
            const res = await ApiCaller.get(
                ROUTES.Order.findOrderNeedGrant,
                this.filter,
            );
            loader.hide();
            if (res.status != 200) {
        ...
        async grantStaffToOrder(order) {
            const payload = {
                id: order.id,
                staffIdBuy: order.staffIdBuy,
            };
            const loader = this.$loading.show();
            const res = await ApiCaller.post(ROUTES.Order.updateOrderStatus, payload);
            loader.hide();
            if (res.status == 200) {
                this.$toast.success(
                    `Gán NV mua cho đơn hàng ${order.orderCode} thành công`,
                    {
                        title: "Thông báo",
                        position: "top-right",
                        autoHideDelay: 7000,
