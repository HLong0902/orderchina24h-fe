...
export default {
	...
	methods: {
		async getListOrders() {
			...
			const res = await ApiCaller.get(ROUTES.Order.findOrderNeedGrant, this.filter);
			loader.hide();
			if (res.status != 200) {
		...
		getStaffById(staffId) {
			...
			else return this.commonStore.staffsBuy?.filter($ => $.id == staffId)[0]?.fullName
		},
		...
		async grantStaffToOrder(order) {
			...
			loader.hide();
			if (res.status == 200) {
                this.$toast.success(`Gán NV mua cho đơn hàng ${order.orderCode} thành công`, {
                    title: 'Thông báo',
                    position: 'top-right',
                    autoHideDelay: 7000,
                })
