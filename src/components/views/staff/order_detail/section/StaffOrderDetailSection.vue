            ...
            <table class="table borderless no_margin">
              <tbody>
                ...
                <tr v-if="CommonUtils.getRole() != CONSTANT.ROLE.NHAN_VIEN_KHO">
                  ...
                  <td>
                    <input
                      :disabled="CommonUtils.getRole() == CONSTANT.ROLE.NHAN_VIEN_TU_VAN"
                      @click="toggleWoodWork"
                ...
                <tr>
                  ...
                  <td>
                    <strong class="big"
                      ...
                      >
                        <div
                          style="font-size: 14px; font-weight: 400; margin: 0; padding: 0"
                        >
                          <table>
                            <tbody>
                              <tr>
                              ...
                              <tr>
                                ...
                                <td style="padding: 5px; text-align: right">
                                  ...
                                  {{ order ? order.orderChina.dateOfPurchase : "-" }}
                                </td>
                              </tr>
                              <tr>
                                <td style="padding: 5px; text-align: left">
                                  <b>Hàng đã về kho TQ</b>
                                </td>
                                <td style="padding: 5px; text-align: right">
                              ...
                              <tr>
                                <td style="padding: 5px; text-align: left">
                                  <b>Đang vận chuyển TQ - VN</b>
                                </td>
                                <td style="padding: 5px; text-align: right">
                                  {{ order?.orderChina?.userOfChinaInventory }}
                                  -
                                  {{
                                    order ? order.orderChina.dateOfChinaInventory : "-"
                                  }}
                                </td>
                              </tr>
                              <tr>
                                <td style="padding: 5px; text-align: left">
                                  <b>Hàng đã về kho VN</b>
                                </td>
                                <td style="padding: 5px; text-align: right">
                                  {{ order?.orderChina?.userOfVietNamInventory }}
                                  -
                                  {{
                                    order ? order.orderChina.dateOfVietNamInventory : "-"
                                  }}
                                </td>
                              </tr>
                              <tr>
                              ...
                              <tr>
                                <td style="padding: 5px; text-align: left"><b>Hủy</b></td>
                                <td style="padding: 5px; text-align: right">
            ...
            <table class="table borderless no_margin">
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                    <strong class="big"
                      >{{ order.orderChina.purchaseFeePerSent }}
                    </strong>
                    ...
                    >
                      <div
                        v-if="
                          !order?.orderLogsUpdateInformation?.some(
                            (el) => el.code == CONSTANT.ORDER_LOGS_CODE.PHI_DICH_VU
                          )
                        "
                        style="
                          font-size: 14px;
                      ...
                      >
                        <div>
                          <span class="bold"
                            >Phí dịch vụ sẽ tính theo tổng tiền hàng</span
                          >
                        </div>
                        <div>
                      ...
                      <div v-else>
                        <span v-if="order.orderChina.purchaseFeePerSent <= 0.00001"
                          >Sử dụng phí riêng được thiết lập cho khách hàng</span
                        >
                        <span
                          v-else
                          v-for="(log, idx) in order.orderLogsUpdateInformation"
                        >
                          <span v-if="log.code == CONSTANT.ORDER_LOGS_CODE.PHI_DICH_VU">
                            {{ log.log }}
                          </span>
                        </span>
                      </div>
                      ...
                      >
                        <div>
                          <span
                            v-if="
                              !order?.orderLogsUpdateInformation?.some(
                                (el) => el.code == CONSTANT.ORDER_LOGS_CODE.GIA_VAN_CHUYEN
                              )
                            "
                            class="bold"
                          >
                            Biểu phí vận chuyển theo
                            {{
                        ...
                        <table>
                          <tbody>
                            ...
                            >
                              <tr
                                v-if="!order?.orderChina?.isVolume"
                            ...
                            <span v-else>
                              <tr
                                v-for="(
                                  log, idx
                                ) in order.orderLogsUpdateInformation.filter(
                                  ($) => $.code == CONSTANT.ORDER_LOGS_CODE.GIA_VAN_CHUYEN
                                )"
                              >
                                <span>
                                  {{ log.log }}
            ...
            <table class="table borderless no_margin">
              <tbody>
                    ...
                    <span class="big">
                      (<span class="red big">{{
                        CommonUtils.formatNumberFloat(
                          order?.orderChina?.foreignCurrencyFees
                        )
                      }}</span>
                      NDT)
                    </span>
                ...
                <tr>
                  ...
                  <td>
                    <span class="big">{{ order?.orderChina?.shippingPrice }}</span>
                    đ (<span class="red big">{{
                ...
                <tr>
                  <td><strong>Đóng gỗ</strong></td>
                  <td>
                    {{ CommonUtils.formatNumber(order.orderChina.woodworkingFee) }} đ
                  </td>
                </tr>
                <tr>
          ...
          <table class="table borderless no_margin">
            <tr>
              ...
              <td width="5%">
                <span class="bold"
                  >Giá vận chuyển /
                  {{ order?.orderChina?.isVolume == true ? "Khối" : "KG" }}</span
                >
              </td>
              <td width="5%">
            ...
            <tr>
              <td>
                <span style="display: flow-root">
                  <input
                    style="width: 80%"
                    v-model="formAdmin.purchaseFeePerSent"
                    size="6"
                    value="0"
                    type="number"
                  />
                  &nbsp;
                  <a class="button-link" @click="handlepurchaseFee(order.orderChina)"
                    >Lưu</a
                  >
                </span>
              </td>
              <td>
                <span style="display: block">
                  <input
                    style="width: 80%"
                    v-model="formAdmin.internationalShippingFees"
                  ...
                  />
                  &nbsp;
                  <a class="button-link" @click="handleShippingPrice(order.orderChina)"
                    >Lưu</a
                  >
                </span>
              </td>
              <td>
                <span style="display: block">
                  <input
                    style="width: 80%"
                    v-model="formAdmin.exchangeRate"
                    @input="formatExchangeRage"
                    size="6"
                    value="0"
                    type="text"
                  />
                  &nbsp;
                  <a class="button-link" @click="handleExchangeRate(order.orderChina)"
                    >Lưu</a
                  >
                </span>
              </td>
              <td>
                <span style="display: table-caption">
                  <select
                    v-model="order.orderChina.status"
              ...
              <tr>
                <td colspan="2">
                  <h4>
                    Số dư hiện tại :
                    <span class="bg_green">{{
                      CommonUtils.formatNumber(info.customerDTO.availableBalance)
                    }}</span>
                    vnđ
                  </h4>
                  <a
        ...
        >
          ...
          <div class="col-md-12">
            <div class="button_confirm clearfix">
              <div>
                <a
                  v-if="CommonUtils.getRole() === CONSTANT.ROLE.ADMIN"
                  @click="openModal('naptien')"
                ...
                <a
                  v-if="CommonUtils.getRole() === CONSTANT.ROLE.ADMIN"
                  @click="openModal('ruttien')"
                  class="button-link special-blue"
                  >Tạo GD rút tiền >></a
                >
                &nbsp;
                <a
                ...
                >
                &nbsp;
                <a @click="openModal('add-addons')" class="button-link special-orange"
                  >Thêm chi phí khác >></a
                >
              </div>
              <b-modal
              ...
              <b-modal
                hide-header-close
                title-html="Nạp tiền vào ví khách hàng"
                id="naptien"
              >
                <div style="font-size: 1rem">
                  <span class="green">Mã giao dịch: </span
                  ><span class="bold">{{ naptien.id }}</span>
                  ...
                  <div>
                    <span class="red">Số tiền: &nbsp;</span>
                    <input
                      @input="formatInputNap"
                      v-model="naptien.amount"
                      placeholder="0"
                      size="20"
                      type="number"
                    />&nbsp;&nbsp;VNĐ
                  ...
                  <b>VNĐ</b>
                  <div v-if="errors.amount" class="bubble-message">
                    {{ errors.amount }}
                  </div>
                  <br />
                  <br />
                  <div class="form-group">
                    <label for="comment" class="bold control-label col-sm-2"
                      >Ghi chú:
                    </label>
                    <textarea
                      v-model="naptien.description"
                      rows="5"
                      name="payment_note"
                      class="inputAccount form-control"
                    ></textarea>
                  </div>
                </div>
                <template #modal-footer>
              ...
              <b-modal hide-header-close title-html="Tạo GD rút tiền" id="ruttien">
                <div style="font-size: 1rem">
                  <span class="green">Mã giao dịch: </span
                  ><span class="bold">{{ ruttien.id }}</span>
      ...
      <div class="cu-row col-md-12" style="display: flex">
        <div class="col-md-6">
          ...
          <a
            ...
            class="button-link special-gray"
          >
            Huỷ đơn </a
          >&nbsp;
          <form
            ...
            method="POST"
          >
            <a class="button-link special-green" @click="buyOrder"> Đã mua hàng </a>
            <div class="ajax_response alert dismissable"></div>
          </form>
          <a
            v-else
          ...
          <tbody>
            ...
            <tr>
              <td colspan="8">
                ...
                <span class="blue">
                  {{
                    order.orderDetails.length > 0 ? order.orderDetails[0].sellerId : "-"
                  }}</span
                >
              </td>
            </tr>
            ...
            >
              ...
              <td>
                <div class="image">
                  <a :href="detail.itemLink" target="_blank"
                    ><img :src="detail.itemImage" width="100px" height="100px"
                  /></a>
                </div>
                <div class="info">
                  <div class="price">
              ...
              <td>
                ...
                <span class="bold">{{
                  CommonUtils.formatNumberFloat(parseFloat(detail?.itemPriceFix))
                }}</span>
                <input
                  v-if="
                    (CommonUtils.getRole() == CONSTANT.ROLE.ADMIN ||
                ...
                <div
                  ...
                  "
                  v-if="
                    (response.originPrice != null || response.originPrice != undefined) &&
              ...
              <td>
                ...
                <p>
                  <a
                    ...
                    @click="handleOutOfProduct(detail)"
                    >Hết hàng</a
                  >
                  <a class="button-link button_special" v-if="detail.status == 0"
              ...
              <td v-if="index == 0" rowspan="4" class="specials">
                ...
                <div>
                  <div class="parent-div ghost">
                    <a class="prefix-text" target="_blank"
                      >Mã shop: <span class="bold"></span
                    ></a>
                    <input
                      class="style-input label_edit"
                      v-if="
                        order.orderChina.status != 0 &&
                        order_shop_code.filter(($) => $ != null).length < 1
                      "
                      type="text"
                      value=""
                      v-model="shopId"
                ...
                >
                  ...
                  <input
                    ...
                    :readonly="CommonUtils.getRole() == CONSTANT.ROLE.NHAN_VIEN_KHO"
                    @keyup.enter.prevent="saveCompanyPayment"
                    class="label_edit"
                  />
                ...
                >
                  <a
                    ...
                    v-if="
                      ...
                      order?.orderChina?.paymentCompany != 0 &&
                      order.orderChina.paymentCompanyDescriptionStaff == null
                    "
                ...
                <div v-if="order.orderChina.status != 0">
                  <div class="parent-div ghost">
                    <a class="prefix-text" target="_blank"
                      >Thêm mã shop: <span class="bold"></span
                    ></a>
                    <input
                      type="text"
                      value=""
                  ...
                  <div v-for="(item, idx) in order_shop_code_complement">
                    <div class="parent-div-short ghost" v-if="item">
                      <a class="prefix-text"> Mã shop: </a>
                      <input
                        class="style-input label_edit"
                        :value="item"
                        type="text"
                        @keyup.enter.prevent="
                          updateShopId($event.target.textContent, idx)
                        "
                        @keypress="preventEnter"
                      />
                    </div>
                ...
                >
                  <div class="vandon_form">
                    ...
                    ><input
                      ...
                      @keyup.enter.prevent="createPackage"
                ...
                <div style="width: 80%; margin-left: 10%" v-if="packages.length > 0">
                  <span v-for="(pkg, it) in packages">
                    <span class="blue">
                      <span :class="promptStyleByPackageStatus(pkg.status)"
                        >{{ pkg.shipCode }}
                    ...
                    <span style="text-align: center">
                      <input
                        v-if="pkg.status == 1"
                        @click="deletePackageById(pkg.id)"
                ...
                <div class="big">
                  ...
                  <span>Giao : </span>
                  {{
                    order?.packages.filter(
                      ($) => $.status == CONSTANT.PACKAGE_STATUS.DA_GIAO
                    ).length
                  }}/{{
                    order?.packages.filter(
                      ($) => $.status == CONSTANT.PACKAGE_STATUS.DA_GIAO
                ...
                <br />
                <div>
                  <a class="bt_yellow" @click="() => (isHideNote = !isHideNote)"
                    >Thêm ghi chú</a
                  >
                  <br />
                  <textarea
                    v-if="!isHideNote"
                    v-model="order.orderChina.orderNote"
                    class="note_first"
                    name="order_note"
                    rows="4"
                    cols="30"
                  ...
                  <a
                    v-if="!isHideNote"
                    class="button-link"
                    @click="handleOrderNote(order.orderChina)"
                    >Lưu</a
                  >
                </div>
              </td>
            ...
            <tr>
              <td colspan="5" class="total">
                <div>
                  <div class="right">
                    Sản phẩm :
                    <span class="green">{{ order.orderChina.totalProduct }}</span>
                    | Tổng tiền :
                    <span class="red">{{
                      CommonUtils.formatNumberFloat(
                        parseFloat(
                          CommonUtils.removeCommas(order.orderChina.totalMoneyNDT)
                        )
                      )
                    }}</span>
                    ( Thực mua :
            ...
            <table>
              <tbody>
                ...
                <tr v-for="(pkg, index) in packages">
                  ...
                  <td>
                    <span class="bold"
                      >{{ pkg.weigh && pkg.weigh > 0 ? pkg.weigh : "-" }}
                      {{ pkg.weigh && pkg.weigh > 0 ? "kg" : "" }}</span
                    >
                  </td>
                  <td>
                    <span class="bold"
                  ...
                  <td>
                    <span class="bold">{{
                      pkg.quantity && pkg.quantity > 0 ? pkg.quantity : "-"
                    }}</span>
                  </td>
                  <td>
                    <span
                  ...
                  <td>
                    ...
                    <!--     </span> -->
                    <!-- </b-tooltip> -->
                    <a class="custom-link hasTooltip tooltipstered"
                      >Chi tiết
              ...
              <tbody>
                ...
                <tr v-for="(transaction, index) in order.transactionHistory">
                  ...
                  <td v-if="transaction.byAdmin !== null && transaction.byAdmin !== ''">
                    ...
                    <span
                      :class="{
                        red: transaction.byAdmin != 1,
                        green: transaction.byAdmin == 1,
                        bold: true,
                      }"
                    >
                      {{ CommonUtils.formatNumber(transaction.amount) }}
              ...
              <tbody>
                ...
                <tr v-for="(itm, id) in order.complains">
                  ...
                  <td>
                    <div class="image">
                      <a :href="itm.orderItemComplain.itemLink" target="_blank"
                        ><img
                          :src="itm.orderItemComplain.itemImage"
                          width="100px"
                          height="100px"
                      /></a>
                    </div>
                    <div class="info">
                      <div class="price">
                        <a :href="itm.orderItemComplain.itemLink" target="_blank">
                          ...
                          <span v-else>
                            <span v-if="!readMore">
                              {{
                                decodeURIComponent(itm.orderItemComplain.itemLink).slice(
                                  0,
                                  100
                                )
                              }}
                      ...
                      <div>
                        <span>
                          <span class="bold">Khiếu nại: </span
                          >{{ itm.complainDescription }}
                        </span>
                      </div>
                    </div>
                  </td>
                  ...
                  <td>
                    <span
                      :class="
                        CommonUtils.promptComplainStatusNameByValue(itm.status).class
                      "
                    >
                      <b>{{
                        CommonUtils.promptComplainStatusNameByValue(itm.status).value
                      }}</b>
                    </span>
                  </td>
                  <td>
                  ...
                  <td>
                    <div class="form-group">
                      <label class="control-label float-left" for="statusComplain"
                        >Trạng thái:</label
                      >
                      <br />
                      <div>
                        ...
                        >
                          ...
                          <option
                            v-for="item in CONSTANT.COMPLAIN_STATUS"
                            :key="item.status"
                            :value="item.status"
                            :class="item.class"
                          >
...
export default {
  ...
  methods: {
    ...
    formatInputRut() {
      ...
      this.ruttien.amountText = unformattedNumberRut
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    },
    formatInputNap() {
      ...
      // Format the number with commas
      this.naptien.amountText = unformattedNumberNap
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    },
    formatNumber(amount) {
    ...
    async submitNapTien() {
      ...
      if (res.status === 200) {
        ...
        this.$toast.success(
          `Nạp thành công ${this.formatNumber(
            res.data.amount + ""
          )} VNĐ vào tài khoản khách hàng.`,
          {
            title: "Thông báo",
            position: "top-right",
            autoHideDelay: 7000,
    ...
    async submitRutTien() {
      ...
      if (res.status === 200) {
        ...
        this.$toast.success(
          `Rút thành công ${this.formatNumber(
            res.data.amount + ""
          )} VNĐ từ tài khoản khách hàng.`,
          {
            title: "Thông báo",
            position: "top-right",
            autoHideDelay: 7000,
    ...
    async getListOrderShopCode(orderId) {
      const loader = this.$loading.show();
      const res = await ApiCaller.get(ROUTES.OrderShopCode.findByOrderIdCustom(orderId));
      loader.hide();
      if (res.status != 200) {
        this.$toast.error(`${res.data.message}`, {
          title: "Thông báo",
    ...
    formatDomesticFees(index) {
      let unformattedNumber = this.order_shop_code[index].domesticFees.replace(/,/g, "");
      this.order_shop_code[index].domesticFees = unformattedNumber
        .toString()
        .replace(/\B(?=(\d{3})+(?!\d))/g, ",");
    },
    formatDomesticFeesReal(index) {
      let unformattedNumber = this.order_shop_code[index].domesticFeesReal.replace(
        /,/g,
        ""
      );
    ...
    async handleSettleOrder() {
      let a =
        this.order?.orderChina?.status != 1
          ? CommonUtils.formatNumber(this.order?.orderChina?.notPaid)
          : CommonUtils.formatNumber(this.order.orderChina?.totalAmount);
      if (a === 0) return;
      const loader = this.$loading.show();
      const payload = {
    ...
    async handleExchangeRate(orderChina) {
      ...
      const payload = {
        orderId: orderChina.id,
        exchangeRate: parseFloat(CommonUtils.removeCommas(this.formAdmin.exchangeRate)),
      };
      const res = await ApiCaller.post(ROUTES.Order.updateFee, payload);
      if (res.status == 200) {
        this.$toast.success(
    ...
    formatShippingPrice() {
      if (this.order.orderChina.shippingPrice) {
        let unformattedNumber = (this.order.orderChina.shippingPrice + "").replace(
          /,/g,
          ""
        );
    ...
    formatExchangeRage() {
      if (this.order.orderChina.exchangeRate) {
        let unformattedNumber = (this.order.orderChina.exchangeRate + "").replace(
          /,/g,
          ""
        );
    ...
    viewTransDetail(id) {
      window.open(
        this.$router.resolve({ name: "StaffTransactionDetailPage", params: { id: id } })
          .href,
        "_blank"
      );
    },
    ...
    async updateComplainStatus(complain) {
      ...
      if (res.status == 200) {
        this.$toast.success(`Cập nhật trạng thái khiếu nại cho đơn hàng thành công`, {
          title: "Thông báo",
          position: "top-right",
          autoHideDelay: 7000,
        });
